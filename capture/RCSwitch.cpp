...
#endif
  {360, {1, 24}, {1,  2}, {2,  1}, false},
};

bool RCSwitch::gotSeries = false;
...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
     ...
     */
    const unsigned int firstDataTiming = 3; //(pro.invertedSignal) ? (3) : (1);
    ...
    // last bit
    code <<= 1;
    if(diff(RCSwitch::timings[changeCount-1], delay * pro.one.high) < delayTolerance) {
      code |= 1;
    }
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
  ...
  if (duration > RCSwitch::nSeparationLimit) {
    ...
    if ((repeatCount==0) || (diff(duration, RCSwitch::timings[0]) < 1000)) {
...
unsigned int* RCSwitch::getSeries() {
  gotSeries = false;
  return RCSwitch::timings;
}
